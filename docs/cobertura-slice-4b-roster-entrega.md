...
## Verificação final — tarefas 3.2 e 3.3

### 3.2 — validação da mudança

`openspec validate slice-4b-roster-entrega --strict` → `Change 'slice-4b-roster-entrega' is valid`,
código de saída 0. CLI local, versão 1.9.0.

Nota de instrumento: `npx openspec@latest validate ... --strict` saiu com código 1 e **sem uma linha
de saída** — nem erro, nem mensagem. Não foi investigado porque não é o instrumento do projeto; fica
registrado para ninguém tomar aquele silêncio por reprovação.

### 3.3 — as negativas, arquivo a arquivo

Baseline: `45f4b59`. `git diff 45f4b59 --name-status` devolve **sete** arquivos, e nada além deles:

```
M  apps/api/src/main/kotlin/com/platos/api/exam/ExamQueries.kt
M  apps/api/src/main/kotlin/com/platos/api/http/Routes.kt
M  apps/api/src/main/kotlin/com/platos/api/http/dto/ExamDto.kt
A  apps/api/src/test/kotlin/com/platos/api/exam/RosterQueryTest.kt
M  apps/api/src/test/kotlin/com/platos/api/http/ExamPackageRouteTest.kt
A  docs/cobertura-slice-4b-roster-entrega.md
M  openspec/changes/slice-4b-roster-entrega/tasks.md
```

Cada negativa medida por conta própria, e não pela leitura da lista acima:

| negativa da proposta | `git diff --name-only` |
|---|---|
| `apps/android` | 0 arquivos |
| `apps/web` | 0 arquivos |
| `packages/domain` | 0 arquivos |
| `vision/` | 0 arquivos |
| `omr/` | 0 arquivos |
| nenhuma spec principal | 0 arquivos em `openspec/specs/` |

Delta único, e em `exam-package`: `openspec/changes/slice-4b-roster-entrega/specs/exam-package/spec.md`
é o único `spec.md` sob `specs/` da mudança. Ele não aparece no diff porque entrou no próprio
`45f4b59`, que é o baseline.

**Nenhuma exceção a nomear.** A negativa não é larga: as seis linhas acima foram medidas uma a uma.

## Verificação final — tarefa 3.1

`./gradlew build --rerun-tasks`. `BUILD SUCCESSFUL in 1m 50s`, 173 tarefas executadas — e o código
de saída 0 é justamente o número em que não se confia. Os números abaixo vêm dos XML dos relatórios.

| módulo :: tarefa | testes | falhas | erros | ignorados |
|---|---|---|---|---|
| `apps/android` :: `testDebugUnitTest` | 233 | 0 | 0 | 0 |
| `apps/api` :: `test` | 140 | 0 | 0 | 0 |
| `packages/domain` :: `jvmTest` | 315 | 0 | 0 | 0 |
| `packages/domain` :: `jsNodeTest` | 307 | 0 | 0 | 0 |
| `packages/domain` :: `testAndroidHostTest` | 307 | 0 | 0 | 0 |
| **total** | **1302** | **0** | **0** | **0** |

138 relatórios, todos com `timestamp` dentro de uma janela de 56 segundos:
**2026-09-11T23:14:43.062Z** a **2026-09-11T23:15:39.318Z**.

A metade instrumentada não foi exigida por esta tarefa, e a 3.3 mostra por quê: `apps/android` tem
**0 arquivos** no diff.

### O relatório velho que a varredura encontrou

Um XML ficou **fora** da janela: `apps/android/.../testReleaseUnitTest/TEST-...RendererContractTest.xml`,
com `timestamp` **2026-08-15T20:29:29.730Z** — quase um mês. É resto de uma execução antiga de
`testReleaseUnitTest`, tarefa que `build` não invoca, e portanto `--rerun-tasks` não a regenerou.

Ele não entra em nenhum número acima, e o motivo de registrá-lo é o tamanho do engano que ele
causaria: a variante `debug` da **mesma classe** rodou nesta rodada com **9** testes, e o arquivo
velho declara **6**. Somar por diretório, sem olhar `timestamp`, produziria um total maior e uma
cobertura menor do que a real — com os dois erros se cancelando parcialmente e nenhum sintoma.
É exatamente o caso que pôs a emenda no `rigorous.md`: para relatório de teste a âncora é o
`timestamp`, nunca a contagem.

## Itens fora de escopo, com dono e fatia-limite (P19)

Nenhum destes foi tocado nesta fatia. Estão aqui para não ficarem só no corpo de um commit.

1. **`listPublished` e `findPackage` não têm cenário direto nenhum.** Só a suíte de rota as
   exercita, e o predicado `EXAM.ORGANIZATION_ID` das duas sobreviveria à remoção sob a suíte atual
   — a mesma lacuna que a mutação (B) desta fatia encontrou em `findRoster`. **Dono:** esta base.
   **Fatia-limite:** a próxima fatia que tocar qualquer uma das duas consultas.
2. **A KDoc de `healthRoutes` manda rodar `curl -sI`, que não funciona.** `HEAD` não é suportado na
   rota e devolve **405**; o comando correto (`curl -s -D - -o /dev/null`) já está em
   `docs/deploy-api.md`, corrigido quando o 405 foi medido em produção. O KDoc ficou com a instrução
   velha — uma linha, nenhum comportamento. Não corrigido aqui porque é arquivo fora do escopo desta
...
- **A rota rodando contra Supabase real.** Os cenários usam Postgres com as migrations aplicadas, e
  não a instância hospedada. A política RLS é a mesma — ela vem das migrations —, mas o
  `identification_mode` de uma organização real e o volume de um roster real não foram observados.
  Fica para a β, que é quando um roster real passa a descer para um aparelho.
- **O custo da leitura do pacote como oráculo de existência.** A decisão está no KDoc da rota, com a
  alternativa nomeada; o número não foi medido. Medi-lo agora seria inventar consumidor para o
  número (P18): ninguém reclamou de latência nesta rota, que ainda não tem cliente.
- **Roster grande.** Nenhum cenário passa de duas linhas. A proposta já registra que paginação sem
  medição seria número sem consumidor.
- **`enrollment_id` em organização `coded`.** O cenário da minimização roda em `nominal`, que é o
  único modo em que matrícula pode ser gravada. Em `coded` o banco a recusa na escrita (ADR-0012), e
  essa recusa tem cobertura própria desde a fatia da LGPD — não foi reexercitada aqui.

## As duas negativas que esta fatia precisa afirmar por nome

**Esta fatia não cria cópia de dado pessoal fora do servidor.** Ela abre o caminho para a **β**
criar. Nada em `apps/android` foi tocado (medido na 3.3: 0 arquivos), não há cache, não há
persistência no aparelho, e a resposta da rota não é gravada em lugar nenhum — a fatia só **lê**.

**As duas linhas de ponto de não-retorno do §16 têm fatia-limite na β, e não aqui.** São a classe H
não enumerar o roster baixado, e a regra de apagamento no aparelho. As duas pressupõem roster
**baixado**, que é coisa que só passa a existir na β. O que esta fatia faz a favor delas é
**minimizar o que desce**: turma e matrícula ficam no servidor, e a mutação (A) mostrou que a
afirmação é executável — com os dois campos reinjetados, a varredura do JSON cai com a mensagem
`` `9Z-noturno` saiu do servidor ``.

## O que esta fatia ensinou sobre o método

O plano previu duas mutações na seção 1. A primeira bateu exatamente. A segunda previu **zero
vermelhos** — e acertar uma previsão de zero não é bom sinal sobre o código: é a descoberta de que
os cenários pedidos pela própria tarefa não conseguiam ver o predicado que a consulta afirma aplicar.
O quinto cenário existe por causa dela.

E a declaração antes da injeção pagou uma segunda vez, de um jeito não previsto: ao escrever "o que
exatamente vai ficar vermelho na mutação (A)", ficou claro que a varredura recém-comitada
**sobreviveria** à mutação, porque nomeava os dois campos à mão. O bloco de declaração antecipada
entrou nesta base por causa de um erro de ancoragem na 4a; aqui ele encontrou uma asserção vácua
antes de ela passar por verde convincente.
