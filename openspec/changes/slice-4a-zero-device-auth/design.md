...
**A exigência é avaliada por `Provider`, e não em tempo de configuração.** A primeira versão falhava ao configurar o projeto, e o Gradle configura **todo** projeto do build — então `:apps:api:test` e `:apps:api:installDist`, que não produzem APK nenhum, passaram a exigir configuração do Android. Não foi hipótese: o workflow que publica a imagem da API quebrou por isso. Com os valores dentro de um `Provider` entregue a `variant.buildConfigFields`, eles só são lidos quando `generateDebugBuildConfig` roda, e a exigência vale para quem realmente monta o aplicativo. O custo é a mensagem sair aninhada sob duas linhas de "Failed to calculate the value of…" do Gradle; a lista do que falta continua legível.
