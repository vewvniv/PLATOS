...
- [x] 5.4 **Tarefa acrescentada durante a implementação, e o motivo é este:** a spec desta mudança
  manda sair apagar a visão junto com a credencial, a escolha e os pacotes, com cenário próprio
  ("Sair apaga a visão"), e **nenhuma das 29 tarefas o nomeava** — a seção 5 falava
  ...
  da mudança funcional, e não limpeza oportunista (P25). **Onde a regra mora mudou no archive, e o
  motivo fica escrito:** ela estava dentro do requisito `ADDED` da visão, e a spec principal já tem
  um requisito que lista o que sair apaga uma coisa a uma — `Sair apaga a sessão e a organização
  escolhida` —, cujo próprio parágrafo diz que o apagamento é nomeado ali "porque requisito genérico
  é fácil de dar como cumprido sem reler". Sincronizar como estava deixaria a regra dita em **dois**
  requisitos e o que enumera **incompleto**: quem lesse só ele concluiria que a visão sobrevive ao
  logout. O delta passou a declarar `MODIFIED` sobre aquele requisito, carregando o bloco inteiro com
  a visão na lista e o cenário movido sem reescrita; o requisito da visão ficou com um ponteiro de
  uma linha, que é a proteção contra a mesma regra voltar a acumular ali. **Nada do que foi
  implementado ou verificado mudou** — `sair()` já apagava as quatro coisas, e
  `sair_apaga_a_visao_junto_com_o_resto` já afirmava isso. **Ver falhar (mutação C):** `sair()` voltando
