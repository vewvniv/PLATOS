...
- [x] 2.4 Conferir que **o pacote e a listagem não mudaram**: a suíte de rota existente roda sem edição, e o `content_hash` que `ExamPublicationTest` fixa continua o mesmo. Resultado: a negativa "nenhum contrato existente alterado" é medida, e não afirmada.
