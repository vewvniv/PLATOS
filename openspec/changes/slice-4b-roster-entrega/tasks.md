...
- [ ] 1.1 **Contrato antes do consumidor** (regra 1): `ExamQueries` ganha a leitura do roster de uma prova, devolvendo **token e nome de apresentação** e mais nada, com `EXAM.ORGANIZATION_ID` no `where` como `listPublished` e `findPackage` já fazem. Mais o DTO de resposta, ao lado de `ExamSummaryDto`. Resultado: código novo sem rota que o exponha, e nenhum comportamento muda.
